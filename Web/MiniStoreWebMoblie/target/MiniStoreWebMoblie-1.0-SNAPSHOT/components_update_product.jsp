<%-- 
    Document   : components_update_product
    Created on : Jul 15, 2023, 2:47:30 PM
    Author     : daigi
--%>

<%@page import="com.ministore.web.DTO.ProductDTO"%>
...
<!--                            <li>
  ...
  <main id="main" class="main">
      ...
      <form action="UpdateProductsServlet" method="POST" enctype="multipart/form-data">
          <c:set var="erros" value="${requestScope.UPDATE_ERRORS}"/>
          <input type="hidden" name="txtSkuProduct" value="${pr.sku}">
          <div class="form-row">
              <span class="form-label">Type Product</span>
              <select name="txtTypeProduct">
                  <option value="${pr.typeProduct}">${pr.typeProduct}</option> 
                  <c:forEach items="${productTypes}" var="type">
                      <c:if test="${type != pr.typeProduct}" >
                          <option value="${type}" >
                              ${type}
                          </option>
                      </c:if>
          ...
          <div class="form-row">
              <span class="form-label">Name Product</span>   
              <input type="text" name="TxtNameProduct" value="${pr.nameProduct}"> <span>(e.g >=1 chars)</span> </br>
              <c:if test="${not empty erros.NameProductErrorCharracter }">
                  <font color="red">
                  ${erros.NameProductErrorCharracter}
                  </font><br/>
              </c:if>
              </br>
          </div>  
          ...
          <div class="form-row">
              <span class="form-label">Id Supplier</span>
              <select name="txtIdSupplier">
                  <option value="${pr.nameSupplier}">${pr.nameSupplier}</option> 
                  <c:forEach items="${nameSupplier}" var="type">
                      <c:if test="${type != pr.nameSupplier}" >
                          <option value="${type}" >
                              ${type}
                          </option>
                      </c:if>
          ...
          <div class="form-row">
              ...
              <select name="txtStatus">
                  <option value="true" <c:if test="${pr.statusP== true}">selected</c:if>>AVAILABLE</option>
                  <option value="false" <c:if test="${pr.statusP== false}">selected</c:if>>SOLD OUT</option>
                  </select></br>
                  </br>
              </div> 
              ...
              <div class="form-row">
                  ...
                  <img src="data:image/jpeg;base64,${pr.pictureProduct}" alt="Product Image" width="100" height="100">

              </br>
          </div> 
          <div class="form-row">
              <span class="form-label">New Picture</span>
              <input type="file" name="newPictureFile" accept="image/*">
